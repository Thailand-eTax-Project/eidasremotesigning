...
public class SignDocIT {
    ...
    public void testSeedBcfksCredential() throws Exception {
        ...
        String keystorePath = bcfksService.createKeystore(
                KEYSTORE_ALIAS, keyPair.getPrivate(), certificate, KEYSTORE_PASSWORD);
    ...
    private byte[] signDocExpectingSuccess(String documentBase64, String signatureFormat)
            throws Exception {
        MvcResult result = signDoc(documentBase64, signatureFormat);
        assertEquals(200, result.getResponse().getStatus(),
                "signDoc must succeed for signature_format \"" + signatureFormat + "\"; body: "
                        + result.getResponse().getContentAsString());
        JsonNode json = objectMapper.readTree(result.getResponse().getContentAsString());
    ...
    /**
     * Validates format + cryptographic intactness with the DSS validator.
     * Also exercises the validation side of the new DSS classpath (Defect A).
     */
    ...
    private X509Certificate selfSign(KeyPair keyPair) throws Exception {
        X500Name subject = new X500Name("CN=SignDoc IT Signer");
